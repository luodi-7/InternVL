...
class ModelArguments:
    ...
    new_embedding_unfreeze: bool = field(
        default=False,
        metadata={'help': 'Set to True to unfreeze the new_embedding of LLM. Default is False.'},
    )
    new_liner_unfreeze: bool = field(
        default=False,
        metadata={'help': 'Set to True to unfreeze the new_liner of LLM. Default is False.'},
    )
...
def main():
    ...
    # 添加 pause token
    pause_tokens = [f'<pause_{i}>' for i in range(18)]  # 定义 pause token
    num_new_other_tokens=len(token_list)
    num_new_pause_tokens=len(pause_tokens)
    token_list.extend(pause_tokens)  # 将 pause token 添加到 token_list 中
    #更新分词器
    num_new_tokens = tokenizer.add_tokens(token_list, special_tokens=True)
    logger.info(f'Added {num_new_tokens} new tokens to the tokenizer.')
    # 获取 pause token 的 ID 范围
    pause_token_ids = [tokenizer.convert_tokens_to_ids(token) for token in pause_tokens]
    pause_token_start_id = min(pause_token_ids)
    pause_token_end_id = max(pause_token_ids) + 1


    # 打印 pause token 的信息
    logger.info(f'Pause token IDs: {pause_token_ids}')
    logger.info(f'Pause token range: {pause_token_start_id} to {pause_token_end_id}')
    img_context_token_id = tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN)

    # 验证 pause token 是否被正确添加
    ...
    # 保存 pause token 的 ID 范围到模型中
    model.pause_token_start_id = pause_token_start_id
    model.pause_token_end_id = pause_token_end_id
    ...
    if num_new_tokens > 0:
        model.language_model.resize_token_embeddings(len(tokenizer)-num_new_pause_tokens)
        output_embeddings = model.language_model.get_output_embeddings().weight.data
        output_embeddings_avg = output_embeddings[:-num_new_other_tokens].mean(dim=0, keepdim=True)
        output_embeddings[-num_new_other_tokens:] = output_embeddings_avg
        

        # 扩展语言模型的 input embeddings
        original_embedding = model.language_model.get_input_embeddings()
        print(original_embedding.type)
        new_embedding = nn.Embedding(num_new_pause_tokens, original_embedding.embedding_dim)
        ...
        # 扩展语言模型的 output embeddings
        # 获取原始的 lm_head
        original_lm_head = model.language_model.get_output_embeddings()

        # 创建新的 lm_head 用于新 token
        new_lm_head = nn.Linear(
            original_lm_head.in_features,  # 输入维度（隐藏层大小）
            num_new_pause_tokens,          # 新 token 的数量
            device=original_lm_head.weight.device,
            dtype=original_lm_head.weight.dtype,
            bias=False
        )

        # 创建 ExtendedLinear 实例
        ...
        # 保存 pause token 的 ID 范围
        model.pause_token_start_id = original_embedding.num_embeddings
        model.pause_token_end_id = model.pause_token_start_id + num_new_pause_tokens
    ...
    if model_args.use_backbone_lora:
        ...
        print("----------------------------------------backbone---------------------------------")
        print(model.language_model.lm_head.weight.requires_grad)  # 应该是 True
    ...
    print(all(param.requires_grad == True for param in extended_embedding.new_embedding.parameters())) #是false
    print(all(param.requires_grad == False for param in extended_embedding.original_embedding.parameters())) #是true
    extended_lm_head = model.lm_head
    print(all(param.requires_grad == True for param in extended_lm_head.new_linear.parameters())) #是false
    print(all(param.requires_grad == False for param in extended_lm_head.original_linear.parameters())) #是true
    # 如果 new_embedding_unfreeze 为 True，解冻 new_embedding 的参数
    if  model_args.new_embedding_unfreeze:
        # 获取 ExtendedEmbedding 实例
        extended_embedding = model.language_model.get_input_embeddings()

        # 解冻 new_embedding 的参数
        for param in extended_embedding.new_embedding.parameters():
            param.requires_grad = True

        # 冻结 original_embedding 的参数
        for param in extended_embedding.original_embedding.parameters():
            param.requires_grad = False
        # 如果 new_embedding_unfreeze 为 True，解冻 new_embedding 的参数
    if  model_args.new_liner_unfreeze:
        # 获取 ExtendedLinear 实例
        extended_lm_head = model.lm_head

        # 解冻 new_linear 的参数
        for param in extended_lm_head.new_linear.parameters():
            param.requires_grad = True

        # 冻结 original_linear 的参数
        for param in extended_lm_head.original_linear.parameters():
            param.requires_grad = False
    ...
    extended_embedding = model.language_model.get_input_embeddings()
    print(all(param.requires_grad == True for param in extended_embedding.new_embedding.parameters())) #是true
    print(all(param.requires_grad == False for param in extended_embedding.original_embedding.parameters())) #是true


    extended_lm_head = model.lm_head
    print(all(param.requires_grad == True for param in extended_lm_head.new_linear.parameters())) #是true
    print(all(param.requires_grad == False for param in extended_lm_head.original_linear.parameters())) #是true
    # 验证 new_linear 和 original_linear 的 requires_grad
    print("new_linear.weight.requires_grad:", extended_lm_head.new_linear.weight.requires_grad)  # 应该是 True
    print("original_linear.weight.requires_grad:", extended_lm_head.original_linear.weight.requires_grad)  # 应该是 False
