...
class DataTrainingArguments:
    ...
    eval_data_path: Optional[str] = field(
        default=None,
        metadata={'help': 'The path of the annotations file of eval datasets.'},
    )
...
def compute_json_metric(eval_preds, model, tokenizer, eval_data_path, user_input_path, image_root):
    # 初始化评估模型
    device = model.device
    dtype = model.dtype
    
    # 初始化CLIP模型
    clip_model = CLIPModel.from_pretrained("/fs-computility/ai-shen/shared/dilab/model/clip-vit-base-patch32").to(device)
    clip_processor = AutoProcessor.from_pretrained("/fs-computility/ai-shen/shared/dilab/model/clip-vit-base-patch32")
    
    # 初始化Sentence-BERT模型
    sbert_model = SentenceTransformer('/fs-computility/ai-shen/shared/dilab/model/all-MiniLM-L6-v2').to(device)
    ...
    with open(user_input_path, 'r') as f:
        for line in f:
            ...
            user_inputs[data['file_name']] = data['user_input']
    ...
    for data in eval_data:
        try:
            # 处理图像
            image_path = os.path.join(image_root, data['image'].lstrip('/'))
            image = Image.open(image_path).convert('RGB')
            pixel_values = transform(image).unsqueeze(0).to(device, dtype)
            ...
            # 提取meme文本
            r_meme = extract_meme_text(response)
            l_meme = extract_meme_text(conversation[-1]['value'])
            ...
            with torch.no_grad():
                ...
                metrics["clip_r"].append(logits_per_image[0, 0].item())
                metrics["clip_l"].append(logits_per_image[0, 1].item())
            
            # 用户输入标签相似度
            file_name = os.path.basename(image_path)
            user_input = user_inputs.get(file_name, {})
            tag_text = " ".join(f"{k}:{v}" for k, v in user_input.items())
            
            embeddings = sbert_model.encode([r_meme, l_meme, tag_text], convert_to_tensor=True)
            metrics["tag_r"].append(util.pytorch_cos_sim(embeddings[0], embeddings[2]).item())
            metrics["tag_l"].append(util.pytorch_cos_sim(embeddings[1], embeddings[2]).item())
            
            # 原有指标计算
            metrics["similarity"].append(simmodel.compare_similarity(r_meme, l_meme))
            metrics["fluency"].append(fluency_model.evaluate_fluency(r_meme))
            metrics["humor"].append(humor_model.evaluate_humor(r_meme))
            
        except Exception as e:
            print(f"Error processing sample {data['id']}: {str(e)}")
    
    # 计算平均指标
    results = {
        "avg_r_clip": sum(metrics["clip_r"]) / len(metrics["clip_r"]),
        "avg_l_clip": sum(metrics["clip_l"]) / len(metrics["clip_l"]),
        "avg_r_tag": sum(metrics["tag_r"]) / len(metrics["tag_r"]),
        "avg_l_tag": sum(metrics["tag_l"]) / len(metrics["tag_l"]),
        "avg_similarity": sum(metrics["similarity"]) / len(metrics["similarity"]),
        "avg_fluency": sum(metrics["fluency"]) / len(metrics["fluency"]),
        "avg_humor": sum(metrics["humor"]) / len(metrics["humor"])
    }
    
    return results

def write_to_file(label, result, filename="/fs-computility/ai-shen/xueyingyi/metric/output.txt"):
...
def main():
    ...
    trainer = Trainer(
        ...
        compute_metrics=partial(
            compute_json_metric,
            model=model,  # 直接传递训练中的模型实例
            tokenizer=tokenizer,
            eval_data_path=data_args.eval_data_path,
            user_input_path="/fs-computility/ai-shen/xueyingyi/meme/meme/generate/quickmeme/user_input.jsonl",
            image_root="/fs-computility/ai-shen/xueyingyi/quickmeme_inpainting/"
        ),
