...
def compute_iou(box1, box2):
    ...
    """
    try:
        x_min1, y_min1, x_max1, y_max1 = [int(x) for x in box1]
        x_min2, y_min2, x_max2, y_max2 = [int(x) for x in box2]
    except ValueError as e:
        print(f"Invalid box coordinates: {box1} or {box2}. Setting IoU to 0.")
        return 0  # 发生异常时返回 0
...
def extract_boxes_and_text(data):
    pattern = re.compile(r"\[\[(.*?)\]\]:(.*?)(?=\n|$)")
    return pattern.findall(data)

def compute_json_metric(eval_preds, model_path: str, tokenizer_use: AutoTokenizer):
    predictions, labels = eval_preds
    simmodel = SimilarityModel()
    with torch.no_grad():
        result = predictions[0]
        eos_token_id = 151645 if 'loc_cleaned_stage_1' in model_path else 2
        ...
        result = tokenizer_use.batch_decode(result, skip_special_tokens=True)
        labels = tokenizer_use.batch_decode(labels, skip_special_tokens=True)
        
        total_iou, total_similarity, count = 0, 0, 0
        
        for r, l in zip(result, labels):
            write_to_file(l,r)
            r_pairs = extract_boxes_and_text(r)
            l_pairs = extract_boxes_and_text(l)
            
            if not l_pairs:  # 如果 labels 为空，跳过
                continue

            if len(r_pairs) != len(l_pairs):
                continue  # 跳过不匹配的情况
            
            iou_sum, sim_sum = 0, 0
            for (r_box, r_text), (l_box, l_text) in zip(r_pairs, l_pairs):
                r_box = r_box.split(', ')
                l_box = l_box.split(', ')
                
                iou_sum += compute_iou(r_box, l_box)
                sim_sum += simmodel.compare_similarity(r_text.strip(), l_text.strip())

            total_iou += iou_sum / len(l_pairs)
            total_similarity += sim_sum / len(l_pairs)
            count += 1
        
        avg_iou = total_iou / count if count > 0 else 0
        avg_similarity = total_similarity / count if count > 0 else 0
        
        return {"avg_similarity": avg_similarity, "avg_iou": avg_iou}

def write_to_file(label, result, filename="/mnt/afs/xueyingyi/location_cleaned_stage_2/output.txt"):
    with open(filename, "a", encoding='utf-8') as file:  # 使用 "a" 模式追加写入文件
        # 获取当前文件的行数，用于生成序号
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = InternVLChatModel.from_pretrained(
            model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config, _fast_init=False)
    else:
    ...
    trainer = Trainer(
        ...
        compute_metrics=partial(compute_json_metric, model_path=model_args.model_name_or_path, tokenizer_use=tokenizer),
